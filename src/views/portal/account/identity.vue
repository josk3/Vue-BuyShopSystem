		...
		<div class="row">
			...
			<div class="col-12 mb-2">
				<el-card shadow="always" class="box-card" :body-style="{ padding: '0px' }">
					...
					<div v-if="info.identity_status !== 1" class="d-flex align-items-start justify-content-start p-3"
						...
						<div class="media-body align-self-center text-truncate">
							<div class="text-truncate">
								<small
									style="white-space: normal;">{{ $t("user.hint_Supplementary_account_information") }}</small>
							</div>
						</div>
					</div>
				</el-card>
			...
			<el-card class="box-card wpy-card" shadow="never" :body-style="{ padding: '0px' }">
				...
				<div class="row">
					<el-alert v-if="detail && detail.identity_reason && info.identity_status === 3"
						:title="detail.identity_reason" center show-icon type="warning"> </el-alert>
					<div class="col-2"></div>
					<div class="col-8">
						<div style="display:flex;justify-content:center;">
							<el-radio-group :disabled="hold_edit" v-model="detail.identity_country_type"
								@change="typeChange" style="margin-top:20px;">
								<el-radio-button label="inland" :disabled="inland_disabled">
									{{ $t("user.inland") }}
								</el-radio-button>
								<el-radio-button label="outland" :disabled="outland_disabled">{{ $t("user.outland") }}
								</el-radio-button>
							</el-radio-group>
						</div>

						<el-form ref="detail" :model="detail" :disabled="hold_edit" :rules="rules"
							:validate-on-rule-change="true" label-width="170px" class="pl-1 pr-3 pt-3 pb-0">
							<el-form-item prop="identity_account_type">
								<template slot="label">
									{{ $t("comm.category") }}
								</template>
								<el-select v-model="detail.identity_account_type" :placeholder="$t('user.select_type')"
									@change="typeChange" filterable>
									<el-option v-for="item in typeList" :key="item.value" :label="$t(item.text)"
										:value="item.value">
							...
							<!-- 境内企业 -->
							<div class="company-inland"
								v-show="detail.identity_account_type !== 'personal' && detail.identity_country_type === 'inland'">
								...
								<el-form-item :label="$t('user.company_start_date')" prop="company_start_date">
									<el-date-picker v-model="detail.company_start_date" value-format="yyyy-MM-dd"
										type="date" :placeholder="$t('comm.select') + $t('user.company_start_date')">
									</el-date-picker>
								</el-form-item>
								<el-form-item :label="$t('user.company_expire_date')" prop="company_expire_date">
									<el-date-picker v-model="detail.company_expire_date" value-format="yyyy-MM-dd"
										type="date" :placeholder="$t('comm.select') + $t('user.company_expire_date')">
									</el-date-picker>
									<el-button size="mini" @click="companyExpireForever" type="text" plain>
										{{ $t("user.long_effective") }} </el-button>
								</el-form-item>
								<el-form-item :label="$t('user.company_identity_photo')" prop="company_identity_photo"
									ref="company_identity_photo">
									<UploadImgOnce :txt="$t('user.upload_company_identity_photo')" size="sm"
										:disable="hold_edit" :img_url="fullImgUrl(detail.company_identity_photo_url)"
								...
								<el-form-item :label="$t('user.company_scope')" prop="company_scope">
									<el-input type="textarea" :rows="2" v-model="detail.company_scope" maxlength="385"
										show-word-limit></el-input>
								</el-form-item>
								<el-form-item
									v-show="detail.identity_account_type === 'company' && detail.identity_country_type === 'inland'"
									:label="$t('user.company_register_address')" prop="company_register_address">
							...
							<!-- 境外企业 -->
							<div class="company-outland"
								v-show="detail.identity_account_type !== 'personal' && detail.identity_country_type === 'outland'">
								<el-form-item :label="$t('user.company_name')" prop="company_name">
									<el-input v-model="detail.company_name"></el-input>
								</el-form-item>
								<el-form-item :label="$t('user.company_register_country')"
								...
								<el-form-item :label="$t('user.is_own_subsidiary')" prop="is_own_subsidiary"
									...
									<el-radio-group v-model="detail.is_own_subsidiary" @change="showSubsidiary">
										<el-radio :label="true" style="line-height:36px;">{{ $t("status.yes") }}
										</el-radio>
										<el-radio :label="false" style="line-height:36px;">{{ $t("status.no") }}
										</el-radio>
									</el-radio-group>
								</el-form-item>
								<el-form-item v-show="detail.is_own_subsidiary === true" prop="subsidiary_name"
									:rules="detail.is_own_subsidiary ? { required: true, message: this.validMsg('user.subsidiary_name'), trigger: 'blur' } : {}">
									<el-input :placeholder="validMsg('user.subsidiary_name')"
										v-model="detail.subsidiary_name"></el-input>
								</el-form-item>
								<el-form-item v-show="detail.is_own_subsidiary === true" prop="subsidiary_country"
									:rules="detail.is_own_subsidiary ? { required: true, message: this.validMsg('user.subsidiary_country'), trigger: 'blur' } : {}">
									<el-input :placeholder="validMsg('user.subsidiary_country')"
										v-model="detail.subsidiary_country"></el-input>
								</el-form-item>
								<el-form-item :label="$t('user.out_company_start_date')" prop="out_company_start_date">
									<el-date-picker v-model="detail.out_company_start_date" value-format="yyyy-MM-dd"
										type="date"
										:placeholder="$t('comm.select') + $t('user.out_company_start_date')">
									</el-date-picker>
								</el-form-item>
								<el-form-item :label="$t('user.out_company_expire_date')"
									prop="out_company_expire_date">
									<el-date-picker v-model="detail.out_company_expire_date" value-format="yyyy-MM-dd"
										type="date"
										:placeholder="$t('comm.select') + $t('user.out_company_expire_date')">
									</el-date-picker>
									<el-button size="mini" @click="outCompanyExpireForever" type="text" plain>
										{{ $t("user.long_effective") }} </el-button>
								</el-form-item>
								<el-form-item :label="$t('user.company_business_identity_photo')"
									prop="company_business_identity_photo" ref="company_business_identity_photo">
									<UploadImgOnce :txt="$t('user.upload_company_business_identity_photo')" size="sm"
										:disable="hold_edit"
										...
										@img="updateImg($event, 'company_business_identity_photo')"></UploadImgOnce>
								</el-form-item>
								<el-form-item :label="$t('user.company_register_identity_photo')"
									prop="company_register_identity_photo" ref="company_register_identity_photo">
									<UploadImgOnce :txt="$t('user.upload_company_register_identity_photo')" size="sm"
										:disable="hold_edit"
										...
										@img="updateImg($event, 'company_register_identity_photo')"></UploadImgOnce>
								</el-form-item>
								<el-form-item :label="$t('user.company_annual_report_photo')"
									prop="company_annual_report_photo" ref="company_annual_report_photo">
									<UploadImgOnce :txt="$t('user.upload_company_annual_report_photo')" size="sm"
										:disable="hold_edit"
										...
										@img="updateImg($event, 'company_annual_report_photo')"></UploadImgOnce>
								</el-form-item>
								<hr />
								...
								<el-form-item prop="company_position">
									...
									<el-select v-model="detail.company_position" :placeholder="$t('user.select_type')"
										filterable>
										<!-- @change="companyPositionChange"  -->
										<el-option v-for="item in companyPositionList" :key="item.value"
							...
							<div class="personal">
								<el-form-item
									v-show="detail.identity_country_type === 'inland' || detail.identity_account_type === 'personal'"
									:label="detail.identity_account_type === 'company' ? $t('user.legal_person') : $t('user.identity_name')"
									prop="identity_name">
									<el-input v-model="detail.identity_name"></el-input>
								</el-form-item>
								<el-form-item
									v-show="detail.identity_country_type === 'outland' && detail.identity_account_type === 'personal'"
									:label="$t('user.belong_what_country')" prop="select_country">
									<!-- <el-input v-model="detail.belong_what_country"></el-input> -->
									<el-select v-model="detail.select_country" :placeholder="$t('comm.country_name')"
										value-key="iso2" @change="selectCountry" filterable clearable>
										<el-option v-for="item in area_all_list" :key="item.iso2"
											:label="item.name + ' (' + item.iso2 + ')'" :value="item">
											<span style="float: left">{{ item.name }}</span>
											<span
												style="float: right; color: #8492a6; font-size: 13px">{{ item.iso2 }}</span>
										</el-option>
									</el-select>
								</el-form-item>

								<el-form-item prop="personal_identity"
									v-show="detail.identity_country_type === 'outland'">
									<template slot="label">
										{{ $t("user.personal_identity") }}
									</template>
									<el-select v-model="detail.personal_identity" :placeholder="$t('user.select_type')"
										@change="personalIdentityChange" filterable>
										<el-option v-for="item in personalIdentityList" :key="item.value"
											:label="$t(item.text)" :value="item.value">
											<span style="float: left">{{ $t(item.text) }}</span>
										</el-option>
									</el-select>
								...
								<el-form-item
									:label="detail.identity_country_type === 'outland' ? $t('user.identity_number2') : $t('user.identity_number')"
									prop="identity_number">
									<el-input v-model="detail.identity_number"></el-input>
								</el-form-item>
								<el-form-item
									:label="detail.identity_country_type === 'outland' ? $t('user.identity_start_date2') : $t('user.identity_start_date')"
									prop="identity_start_date">
									<el-date-picker v-model="detail.identity_start_date" value-format="yyyy-MM-dd"
										type="date"
										:placeholder="detail.identity_country_type === 'outland' ? $t('comm.select') + $t('user.identity_start_date2') : $t('comm.select') + $t('user.identity_start_date')">
									</el-date-picker>
								</el-form-item>
								<el-form-item
									:label="detail.identity_country_type === 'outland' ? $t('user.identity_expire_date2') : $t('user.identity_expire_date')"
									prop="identity_expire_date">
									<el-date-picker v-model="detail.identity_expire_date" value-format="yyyy-MM-dd"
										type="date"
										:placeholder="detail.identity_country_type === 'outland' ? $t('comm.select') + $t('user.identity_expire_date2') : $t('comm.select') + $t('user.identity_expire_date')">
									</el-date-picker>
									<el-button size="mini" @click="identityExpireForever" type="text" plain>
										{{ $t("user.long_effective") }} </el-button>
								</el-form-item>
								<el-form-item
									:label="detail.identity_country_type === 'outland' ? $t('user.identity_photo_a2') : $t('user.identity_photo_a')"
									prop="identity_photo_a" ref="identity_photo_a">
									<UploadImgOnce
										:txt="detail.identity_country_type === 'outland' ? $t('user.upload_identity_photo_a2') : $t('user.upload_identity_photo_a')"
										size="sm" :disable="hold_edit"
										:img_url="fullImgUrl(detail.identity_photo_a_url)"
										@img="updateImg($event, 'identity_photo_a')" :img_url_demo="getPhotoDemoA()"
										:img_url_tip="setIdentityPhotoTipA()"></UploadImgOnce>
								</el-form-item>
								<el-form-item
									...
									prop="identity_photo_b" ref="identity_photo_b">
									<UploadImgOnce
										...
										:img_url_tip="setIdentityPhotoTipB()"></UploadImgOnce>
								</el-form-item>
								<el-form-item
									...
									prop="identity_photo_c" ref="identity_photo_c">
									<UploadImgOnce
							...
							<!-- 境外企业 地址电话-->
							<div
								v-show="detail.identity_country_type === 'outland' && detail.identity_account_type === 'company'">
								<el-form-item :label="$t('user.company_register_address')"
							...
							<el-form-item v-show="detail.identity_account_type === 'personal'"
								:label="$t('user.personal_address')" prop="address">
								<el-input v-model="detail.address"></el-input>
							</el-form-item>
							<el-form-item
								v-show="detail.identity_country_type === 'inland' && detail.identity_account_type === 'company'"
								:label="$t('user.company_address')" prop="company_address">
								<el-input v-model="detail.company_address"></el-input>
							</el-form-item>
...
<script>
	...
	import {
		setIdentityMessageboxID,
		getIdentityMessageboxID
	...
	export default {
		...
		data() {
			...
			return {
				...
				identity_photo_a_id_demo_url: configs.template.settleBasePath +
					"%E5%A2%83%E5%86%85_%E8%BA%AB%E4%BB%BD%E8%AF%81%E6%AD%A3%E9%9D%A2%2B.jpg",
				...
				rulesB: {
					...
					company_name: [{
						...
						{
							validator: checkInlandCompanyName,
							trigger: "blur"
						},
					],
					...
					company_identity_id: [{
						...
						{
							validator: checkInlandCompanyIdentityId,
							trigger: "blur"
						},
					],
					company_start_date: [{
					...
					company_phone: [{
						...
						{
							validator: checkInlandCompanyPhone,
							trigger: "blur"
						},
					],
		...
		methods: {
			open() {
				...
				if (getIdentityMessageboxID() != false && getIdentityMessageboxID() != 'false' && this.$route.meta.name ==
					"merchant_identity") {
					//是否未认证
					if (this.$data.info.identity_status == 0 || this.$data.info.identity_status == '0') {
						this.$alert(this.$t("user.hint_Supplementary_account_information"), this.$t(
						'dispute.warm_prompt'), {
			...
			showSubsidiary(val) {
				...
				if (this.detail.is_own_subsidiary === true && this.detail.identity_country_type === "outland" && this
					.detail.identity_account_type === "company") {
